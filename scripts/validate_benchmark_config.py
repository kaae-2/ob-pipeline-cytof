...
def output_ids(module_or_stage: dict[str, Any]) -> set[str]:
    return {output.get('id') for output in module_or_stage.get('outputs', []) if output.get('id')}
...
def validate(config_path: Path) -> list[str]:
    ...
    stages = {stage.get('id'): stage for stage in data.get('stages', []) if isinstance(stage, dict)}
    ...
    for stage_id, stage in stages.items():
        ...
        for module in stage.get('modules', []):
            ...
            if env_id and env_id not in environments:
                error(errors, f'module {stage_id}.{module_id} references unknown software_environment: {env_id}')
...
def main() -> None:
    ...
    if errors:
        print('Benchmark config validation failed:')
        for item in errors:
            print(f'- {item}')
        raise SystemExit(1)
